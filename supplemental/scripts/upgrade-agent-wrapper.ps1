...
# Big Brother Agent Upgrade Wrapper
# This script downloads and executes the latest upgrade script from GitHub

$ErrorActionPreference = "Stop"

try {
    Write-Host "Big Brother Agent Upgrade Wrapper" -ForegroundColor Cyan
    Write-Host "============================" -ForegroundColor Cyan
    Write-Host ""
    
    # Define the URL for the latest upgrade script
    $scriptUrl = "https://raw.githubusercontent.com/cyber-dev-skalovsi/big-brother/main/supplemental/scripts/upgrade-agent.ps1"
    $tempScriptPath = "$env:TEMP\bigbrother-upgrade-agent-$(Get-Date -Format 'yyyyMMdd-HHmmss').ps1"
