...
# Function to find bigbrother-agent in common installation locations
function Find-Big BrotherAgent {
    # First check if it's in PATH
    $agentCmd = Get-Command "bigbrother-agent" -ErrorAction SilentlyContinue
    if ($agentCmd) {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\scoop\apps\bigbrother-agent\current\BIGBROTHER-agent.exe",
        "$env:ProgramData\scoop\apps\bigbrother-agent\current\BIGBROTHER-agent.exe",
        "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\cyber-dev-skalovsi.BIGBROTHER-agent*\BIGBROTHER-agent.exe",
        "$env:ProgramFiles\WinGet\Packages\cyber-dev-skalovsi.BIGBROTHER-agent*\BIGBROTHER-agent.exe",
        "${env:ProgramFiles(x86)}\WinGet\Packages\cyber-dev-skalovsi.BIGBROTHER-agent*\BIGBROTHER-agent.exe",
        "$env:ProgramFiles\bigbrother-agent\BIGBROTHER-agent.exe",
        "$env:ProgramFiles(x86)\bigbrother-agent\BIGBROTHER-agent.exe",
        "$env:SystemDrive\Users\*\scoop\apps\bigbrother-agent\current\BIGBROTHER-agent.exe"
    )
...
function Install-Scoop {
    ...
    if (Test-Admin) {
        throw "Scoop cannot be installed with administrator privileges. Please run this script as a regular user first to install Scoop and bigbrother-agent, then run as admin to configure the service."
    }
...
# Function to install bigbrother-agent with Scoop
function Install-Big BrotherAgentWithScoop {
    Write-Host "Adding bigbrother bucket..."
    scoop bucket add bigbrother https://github.com/cyber-dev-skalovsi/big-brother-scoops | Out-Null
    
    Write-Host "Installing / updating bigbrother-agent..."
    scoop install bigbrother-agent | Out-Null
    
    if (-not (Test-CommandExists "bigbrother-agent")) {
        throw "Failed to install bigbrother-agent"
    }
    
    return $(Join-Path -Path $(scoop prefix bigbrother-agent) -ChildPath "BIGBROTHER-agent.exe")
}

# Function to install bigbrother-agent with WinGet
function Install-Big BrotherAgentWithWinGet {
    Write-Host "Installing / updating bigbrother-agent..."
    ...
    & winget install --exact --id cyber-dev-skalovsi.BIGBROTHER-agent --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    # Refresh PATH environment variable to make bigbrother-agent available in current session
    $env:Path = [System.Environment]::GetEnvironmentVariable("Path", "Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path", "User")
    
    # Find the path to the bigbrother-agent executable
    $agentPath = (Get-Command bigbrother-agent -ErrorAction SilentlyContinue).Source
    
    if (-not $agentPath) {
        throw "Could not find bigbrother-agent executable path after installation"
    }
...
function Install-WithScoop {
    ...
    try {
        ...
        # Install bigbrother-agent
        $agentPath = Install-Big BrotherAgentWithScoop
...
function Install-WithWinGet {
    ...
    try {
        ...
        # Install bigbrother-agent
        $agentPath = Install-Big BrotherAgentWithWinGet
...
function Install-NSSMService {
    ...
    Write-Host "Installing bigbrother-agent service..."
    ...
    $existingService = Get-Service -Name "bigbrother-agent" -ErrorAction SilentlyContinue
    if ($existingService) {
        ...
        try {
            $currentPath = & $nssmCommand get bigbrother-agent Application
            if ($LASTEXITCODE -eq 0 -and $currentPath.Trim() -eq $AgentPath) {
                ...
                & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+KEY=$Key"
                if ($TokenProvided) { & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+TOKEN=$Token" }
                if ($UrlProvided) { & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+HUB_URL=$HubUrl" }
                if ($PortProvided) { & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+PORT=$Port" }
                ...
                if ($existingService.Status -eq "Running") {
                    ...
                    & $nssmCommand restart bigbrother-agent
                }
                return
            }
        ...
        try {
            & $nssmCommand stop bigbrother-agent
            & $nssmCommand remove bigbrother-agent confirm
        } catch {
            Write-Host "Warning: Failed to remove existing service: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
    
    & $nssmCommand install bigbrother-agent $AgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to install bigbrother-agent service"
    }
    
    Write-Host "Configuring service environment variables..."
    & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+KEY=$Key"
    & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+TOKEN=$Token"
    & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+HUB_URL=$HubUrl"
    & $nssmCommand set bigbrother-agent AppEnvironmentExtra "+PORT=$Port"
    
    # Configure log files
    $logDir = "$env:ProgramData\bigbrother-agent\logs"
    if (-not (Test-Path $logDir)) {
        New-Item -ItemType Directory -Path $logDir -Force | Out-Null
    }
    $logFile = "$logDir\bigbrother-agent.log"
    & $nssmCommand set bigbrother-agent AppStdout $logFile
    & $nssmCommand set bigbrother-agent AppStderr $logFile
}
...
function Configure-Firewall {
    ...
    $ruleName = "Allow bigbrother-agent"
    ...
    Write-Host "Creating firewall rule for bigbrother-agent on port $Port..."
    try {
...
function Start-Big BrotherAgentService {
    param (
        [string]$NSSMPath = ""
    )
    
    Write-Host "Starting bigbrother-agent service..."
    ...
    & $nssmCommand start bigbrother-agent
    ...
    if ($startResult -ne 0) {
        ...
        while (-not $serviceStarted -and $elapsedTime -lt $maxWaitTime) {
            ...
            $serviceStatus = & $nssmCommand status bigbrother-agent
            
            if ($serviceStatus -eq "SERVICE_RUNNING") {
                $serviceStarted = $true
                Write-Host "Success! The bigbrother-agent service is now running." -ForegroundColor Green
            }
        ...
        if (-not $serviceStarted) {
            ...
            Write-Host "You can check status manually with 'nssm status bigbrother-agent'" -ForegroundColor Yellow
        }
    } else {
        # NSSM start command was successful
        Write-Host "Success! The bigbrother-agent service is running properly." -ForegroundColor Green
    }
}
...
try {
    ...
    if (-not $AgentPath) {
        throw "Could not find bigbrother-agent executable. Make sure it was properly installed."
    }
    ...
    if (-not $isAdmin -and -not $Elevated) {
        ...
        Write-Host "Check service status with 'nssm status bigbrother-agent'"
        Write-Host "Edit service configuration with 'nssm edit bigbrother-agent'"
    ...
    if ($isAdmin -or $Elevated) {
        ...
        # Start the service
        Start-Big BrotherAgentService -NSSMPath $NSSMPath
