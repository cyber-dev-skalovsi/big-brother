...
# Function to upgrade bigbrother-agent with Scoop
function Upgrade-Big BrotherAgentWithScoop {
    Write-Host "Upgrading bigbrother-agent with Scoop..."
    scoop update bigbrother-agent
    
    if (-not (Test-CommandExists "bigbrother-agent")) {
        throw "Failed to upgrade bigbrother-agent with Scoop"
    }
    
    return $(Join-Path -Path $(scoop prefix bigbrother-agent) -ChildPath "BIGBROTHER-agent.exe")
}

# Function to upgrade bigbrother-agent with WinGet
function Upgrade-Big BrotherAgentWithWinGet {
    Write-Host "Upgrading bigbrother-agent with WinGet..."
    ...
    & winget upgrade --exact --id cyber-dev-skalovsi.BIGBROTHER-agent --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    if (-not $agentPath) {
        ...
        $agentPath = Find-Big BrotherAgent
        if (-not $agentPath) {
            throw "Could not find bigbrother-agent executable path after upgrade"
        }
    }
...
function Get-ServiceConfiguration {
    ...
    if (-not $existingService) {
        throw "bigbrother-agent service does not exist. Please run the installation script first."
    }
    ...
    try {
        ...
        $envVars = & $nssmCommand get bigbrother-agent AppEnvironmentExtra
...
function Update-ServicePath {
    ...
    Write-Host "Updating bigbrother-agent service path..."
    ...
    & $nssmCommand set bigbrother-agent Application $NewAgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to update bigbrother-agent service path"
    }
    
    Write-Host "Service path updated to: $NewAgentPath"
    
    # Start the service
    Start-Big BrotherAgentService -NSSMPath $nssmCommand
}
...
try {
    Write-Host "Big Brother Agent Upgrade Script" -ForegroundColor Cyan
    Write-Host "===========================" -ForegroundColor Cyan
    
    # First: Check if service exists (doesn't require admin)
    $existingService = Get-Service -Name "bigbrother-agent" -ErrorAction SilentlyContinue
    if (-not $existingService) {
        Write-Host "ERROR: bigbrother-agent service does not exist." -ForegroundColor Red
    ...
    Write-Host "Stopping bigbrother-agent service..."
    & $nssmPath stop bigbrother-agent
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Upgrading bigbrother-agent..."
    $newAgentPath = $null
    
    if (Test-CommandExists "scoop") {
        Write-Host "Using Scoop for upgrade..."
        $newAgentPath = Upgrade-Big BrotherAgentWithScoop
    }
    elseif (Test-CommandExists "winget") {
        Write-Host "Using WinGet for upgrade..."
        $newAgentPath = Upgrade-Big BrotherAgentWithWinGet
    }
    else {
    ...
    if (-not $newAgentPath) {
        $newAgentPath = Find-Big BrotherAgent
        if (-not $newAgentPath) {
            throw "Could not find bigbrother-agent executable after upgrade."
        }
    }
    ...
    if ($isAdmin -or $Elevated) {
        ...
        Write-Host ""
        Write-Host "Upgrade completed successfully!" -ForegroundColor Green
        Write-Host "The bigbrother-agent service has been updated to use the new executable path." -ForegroundColor Green
