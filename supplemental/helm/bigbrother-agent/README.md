# Big Brother Agent Helm Chart

A Kubernetes Helm chart for deploying [Big Brother Agent](https://github.com/cyber-dev-skalovsi/big-brother/) - a lightweight monitoring agent that collects system metrics and sends them to a central Big Brother Hub.

## Overview

This Helm chart simplifies the deployment of Big Brother Agent in Kubernetes environments. By default, it deploys as a DaemonSet to run one agent on each node in the cluster. The agent monitors node-level system resources (CPU, memory, disk, network, temperature, GPU, etc.) and provides detailed metrics to the Big Brother Hub for centralized monitoring and alerting.
...
- Kubernetes 1.19+
- Helm 3.0+
- Big Brother Hub instance running and accessible
- SSH public key for agent authentication

## What Gets Monitored

In Kubernetes environments, the Big Brother agent monitors **node-level metrics**:
...
```bash
helm install bigbrother-agent oci://ghcr.io/cyber-dev-skalovsi/big-brother-charts/bigbrother-agent \
  --set env.KEY="ssh-ed25519 AAAA... your-public-key" \
  --set env.TOKEN="your-token-value" \
  --set env.HUB_URL="http://bigbrother-hub:8090"
```

Or with custom values:

```bash
helm install bigbrother-agent oci://ghcr.io/cyber-dev-skalovsi/big-brother-charts/bigbrother-agent -f custom-values.yaml
```

### 2. Verify the Agent is Running

```bash
kubectl get pods -l app.kubernetes.io/name=bigbrother-agent
kubectl logs -l app.kubernetes.io/name=bigbrother-agent
```
...
| `env.HUB_URL` | Empty | Hub URL (e.g., http://bigbrother-hub:8090) |
...
```bash
helm install bigbrother-agent ./bigbrother-agent \
  --set env.KEY="ssh-ed25519 AAAA... your-public-key" \
  --set env.TOKEN="your-token-value" \
  --set env.HUB_URL="http://bigbrother-hub:8090"
```
...
# values.yaml
image:
  repository: cyber-dev-skalovsi/big-brother-agent
...
env:
  ...
  HUB_URL: "http://bigbrother-hub:8090"
...
```yaml
image:
  repository: cyber-dev-skalovsi/big-brother-agent-nvidia
...
**Note**: The GPU image (`cyber-dev-skalovsi/big-brother-agent-nvidia`) is specifically for monitoring NVIDIA GPUs on the node. It does not provide container-level GPU metrics.

Or via CLI:

```bash
helm install bigbrother-agent ./bigbrother-agent \
  --set image.repository=cyber-dev-skalovsi/big-brother-agent-nvidia \
...
```yaml
secret:
  existingSecret: my-bigbrother-secret
...
# View agent logs
kubectl logs -l app.kubernetes.io/name=bigbrother-agent -f
...
kubectl rollout restart daemonset bigbrother-agent

# For Deployment (if daemonset.enabled=false)
kubectl rollout restart deployment bigbrother-agent
```

### Uninstall

```bash
helm uninstall bigbrother-agent
```

### View Helm Release History

```bash
helm history bigbrother-agent
helm rollback bigbrother-agent 1  # Rollback to previous version
```
...
| `HUB_URL` | Empty | Hub URL (e.g., http://bigbrother-hub:8090) |
...
- Check DNS resolution: `kubectl exec <pod-name> -- nslookup bigbrother-hub.default.svc.cluster.local`
- Verify SSH key is correctly configured
- Check firewall rules for port 8090 (Hub) and 45876 (Agent)

### GPU Not Detected

- Confirm image is `cyber-dev-skalovsi/big-brother-agent-nvidia`
...
- `HUB_URL` - Hub address (e.g., http://bigbrother-hub:8090 or https://bigbrother.example.com)

To verify the secret was created:

```bash
kubectl get secret bigbrother-agent
kubectl get secret bigbrother-agent -o jsonpath='{.data.ssh-key}' | base64 -d
```

## Support and Documentation

- **Project Homepage**: https://github.com/cyber-dev-skalovsi/big-brother/
- **GitHub Repository**: https://github.com/cyber-dev-skalovsi/big-brother
- **Agent Documentation**: https://github.com/cyber-dev-skalovsi/big-brother/

**Note**: The main Big Brother documentation describes Docker/Podman container monitoring. In Kubernetes, the agent focuses on node-level metrics. For Kubernetes-specific container/pod monitoring, use tools like metrics-server, Prometheus, or the Kubernetes Metrics API.
...
Please refer to the main Big Brother project repository for license information.
