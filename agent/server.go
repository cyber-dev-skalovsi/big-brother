...
func (a *Agent) StartServer(opts ServerOptions) error {
	...
	config := &gossh.ServerConfig{
		ServerVersion: fmt.Sprintf("SSH-2.0-%s_%s", bigbrother.AppName, bigbrother.Version),
	}
...
func (a *Agent) handleSession(s ssh.Session) {
	...
	if hubVersion.LT(bigbrother.MinVersionAgentResponse) {
...
func (a *Agent) writeToSession(w io.Writer, stats *system.CombinedData, hubVersion semver.Version) error {
	if hubVersion.GTE(bigbrother.MinVersionCbor) {
		return cbor.NewEncoder(w).Encode(stats)
	}
	return json.NewEncoder(w).Encode(stats)
}

// extractHubVersion extracts the Big Brother version from SSH client version string.
// Expected format: "SSH-2.0-BIGBROTHER_X.Y.Z" or "BIGBROTHER_X.Y.Z"
