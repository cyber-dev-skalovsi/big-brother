...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/internal/entities/container"
...
func TestStartServer(t *testing.T) {
	...
	socketFile := filepath.Join(t.TempDir(), "BIGBROTHER-test.sock")
...
func TestStartServerDisableSSH(t *testing.T) {
	t.Setenv("BIGBROTHER_AGENT_DISABLE_SSH", "true")
...
func TestExtractHubVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "valid BIGBROTHER client version with underscore",
			clientVersion:   "SSH-2.0-BIGBROTHER_0.11.1",
			expectedVersion: "0.11.1",
			expectError:     false,
		},
		{
			name:            "valid BIGBROTHER client version with beta",
			clientVersion:   "SSH-2.0-BIGBROTHER_1.0.0-beta",
			expectedVersion: "1.0.0-beta",
			expectError:     false,
		},
		{
			name:            "valid BIGBROTHER client version with rc",
			clientVersion:   "SSH-2.0-BIGBROTHER_0.12.0-rc1",
		...
		},
		{
			...
			clientVersion: "SSH-2.0-BIGBROTHER",
			expectError:   true,
		},
		{
		...
		},
		{
			...
			clientVersion:   "BIGBROTHER_",
			expectedVersion: "",
			expectError:     true,
		},
		{
			name:            "version with patch and build metadata",
			clientVersion:   "SSH-2.0-BIGBROTHER_1.2.3+build.123",
...
func TestGetHubVersion(t *testing.T) {
	...
	mockCtx := &mockSSHContext{
		...
		clientVersion: "SSH-2.0-BIGBROTHER_0.12.0",
	}
	...
	mockCtx.clientVersion = "SSH-2.0-BIGBROTHER_0.11.0"
	version = agent.getHubVersion(mockCtx)
	assert.Equal(t, "0.11.0", version.String())

	// Test with invalid version string (non-BIGBROTHER client)
	mockCtx.clientVersion = "SSH-2.0-OpenSSH_8.0"
	version = agent.getHubVersion(mockCtx)
	assert.Equal(t, "0.0.0", version.String()) // Should be empty version for non-BIGBROTHER clients
...
func (m *mockSSHContext) ServerVersion() string {
	return "SSH-2.0-BIGBROTHER_test"
}
...
// hubVersions cache (see https://github.com/cyber-dev-skalovsi/big-brother/issues/2128).
