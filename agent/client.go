...
func (client *WebSocketClient) getOptions() *gws.ClientOption {
	...
	client.hubURL.Path = path.Join(client.hubURL.Path, "api/bigbrother/agent-connect")

	// make sure BIGBROTHER_AGENT_ALL_PROXY works (GWS only checks ALL_PROXY)
	if val := os.Getenv("BIGBROTHER_AGENT_ALL_PROXY"); val != "" {
	...
	client.options = &gws.ClientOption{
		...
		RequestHeader: http.Header{
			...
			"X-BIGBROTHER":   []string{bigbrother.Version},
		},
