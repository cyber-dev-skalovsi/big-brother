...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/agent/deltatracker"
...
func (dm *dockerManager) updateContainerStats(ctr *container.ApiInfo, cacheTimeMs uint16) error {
	...
	// https://github.com/cyber-dev-skalovsi/big-brother/issues/2049
	var cpuPct float64
	if dm.isWindows {
	...
	if err != nil {
		return fmt.Errorf("%s - %w - see https://github.com/cyber-dev-skalovsi/big-brother/issues/144", name, err)
	}
...
func (dm *dockerManager) applyDockerVersionInfo(serverHeader string, versionInfo *dockerVersionResponse) {
	...
	if dockerVersion, err := semver.Parse(versionInfo.Version); err == nil && dockerVersion.Major > 24 {
		...
		slog.Info(fmt.Sprintf("Docker %s is outdated. Upgrade if possible. See https://github.com/cyber-dev-skalovsi/big-brother/issues/58", versionInfo.Version))
	}
}
