...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother"
...
func TestNewWebSocketClient(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			if tc.hubURL != "" {
				t.Setenv("BIGBROTHER_AGENT_HUB_URL", tc.hubURL)
			}
			if tc.token != "" {
				t.Setenv("BIGBROTHER_AGENT_TOKEN", tc.token)
			}
...
func TestWebSocketClient_GetOptions(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/api/bigbrother/agent-connect",
		},
		{
		...
		},
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/custom/path/api/bigbrother/agent-connect",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			t.Setenv("BIGBROTHER_AGENT_HUB_URL", tc.inputURL)
			t.Setenv("BIGBROTHER_AGENT_TOKEN", "test-token")
			...
			assert.Equal(t, bigbrother.Version, options.RequestHeader.Get("X-BIGBROTHER"))
...
func TestWebSocketClient_VerifySignature(t *testing.T) {
	...
	// Set up environment
	t.Setenv("BIGBROTHER_AGENT_HUB_URL", "http://localhost:8080")
	t.Setenv("BIGBROTHER_AGENT_TOKEN", "test-token")
...
func TestGetToken(t *testing.T) {
	...
	t.Run("token from BIGBROTHER_AGENT_TOKEN environment variable", func(t *testing.T) {
		// Set BIGBROTHER_AGENT_TOKEN env var (should take precedence)
		expectedToken := "test-token-from-BIGBROTHER-env"
		t.Setenv("BIGBROTHER_AGENT_TOKEN", expectedToken)
	...
	t.Run("token from BIGBROTHER_AGENT_TOKEN_FILE", func(t *testing.T) {
		// Create a temporary token file
		expectedToken := "test-token-from-BIGBROTHER-file"
		tokenFile, err := os.CreateTemp("", "token-test-*.txt")
		require.NoError(t, err)
		defer os.Remove(tokenFile.Name())
		...
		// Set BIGBROTHER_AGENT_TOKEN_FILE env var (should take precedence)
		t.Setenv("BIGBROTHER_AGENT_TOKEN_FILE", tokenFile.Name())
	...
	t.Run("error when neither TOKEN nor TOKEN_FILE is set", func(t *testing.T) {
		t.Setenv("BIGBROTHER_AGENT_TOKEN", "")
		t.Setenv("TOKEN", "")
		t.Setenv("BIGBROTHER_AGENT_TOKEN_FILE", "")
		t.Setenv("TOKEN_FILE", "")
