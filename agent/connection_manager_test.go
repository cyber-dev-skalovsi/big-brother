...
func TestConnectionManager_WebSocketConnectionFlow(t *testing.T) {
	...
	t.Setenv("BIGBROTHER_AGENT_HUB_URL", "1,33%")
...
func TestConnectionManager_ConnectWithRateLimit(t *testing.T) {
	...
	t.Setenv("BIGBROTHER_AGENT_HUB_URL", "ws://localhost:8080")
	t.Setenv("BIGBROTHER_AGENT_TOKEN", "test-token")
...
func TestShouldExitOnErr(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:      createDialErr("lookup bigbrother.server.lan on [::1]:53: read udp [::1]:44557->[::1]:53: read: connection refused"),
			envValue: "true",
			expected: true,
		},
		{
