...
func TestNewGPUManagerPriorityNvtopFallback(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "nvtop,nvidia-smi")
...
func TestNewGPUManagerPriorityMixedCollectors(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "intel_gpu_top,rocm-smi")
...
func TestNewGPUManagerPriorityNvmlFallbackToNvidiaSmi(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "nvml,nvidia-smi")
...
func TestNewGPUManagerConfiguredCollectorsMustStart(t *testing.T) {
	...
	t.Run("configured valid collector unavailable", func(t *testing.T) {
		t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "nvidia-smi")
		gm, err := NewGPUManager()
		require.Nil(t, gm)
		require.Error(t, err)
	...
	t.Run("configured collector list has only unknown entries", func(t *testing.T) {
		t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "bad,unknown")
		gm, err := NewGPUManager()
		require.Nil(t, gm)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "no configured GPU collectors are available")
...
func TestNewGPUManagerConfiguredNvmlBypassesCapabilityGate(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("BIGBROTHER_AGENT_GPU_COLLECTOR", "nvml")
...
`, argsFile)
	...
	t.Setenv("BIGBROTHER_AGENT_INTEL_GPU_DEVICE", "sriov")
