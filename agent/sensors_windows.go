...
//go:generate dotnet build -c Release lhm/BIGBROTHER_lhm.csproj
...
var (
	BIGBROTHERLhm     *lhmProcess
	BIGBROTHERLhmOnce sync.Once
...
func newlhmProcess() (*lhmProcess, error) {
	destDir := filepath.Join(os.TempDir(), "BIGBROTHER")
	execPath := filepath.Join(destDir, "BIGBROTHER_lhm.exe")
...
func getSensorTemps(ctx context.Context) (temps []sensors.TemperatureStat, err error) {
	...
	BIGBROTHERLhmOnce.Do(func() {
		BIGBROTHERLhm, err = newlhmProcess()
	})
	...
	if BIGBROTHERLhm == nil {
		return temps, fmt.Errorf("lhm not available")
	}

	return BIGBROTHERLhm.getTemps(ctx)
}
