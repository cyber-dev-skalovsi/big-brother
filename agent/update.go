...
// restarter knows how to restart the BIGBROTHER-agent service.
...
func (s *systemdRestarter) Restart() error {
	...
	if err := exec.Command(s.cmd, "is-active", "BIGBROTHER-agent.service").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting BIGBROTHER-agent.service via systemd…")
	return exec.Command(s.cmd, "restart", "BIGBROTHER-agent.service").Run()
}

type openRCRestarter struct{ cmd string }

func (o *openRCRestarter) Restart() error {
	if err := exec.Command(o.cmd, "BIGBROTHER-agent", "status").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting BIGBROTHER-agent via OpenRC…")
	return exec.Command(o.cmd, "BIGBROTHER-agent", "restart").Run()
}
...
func (w *openWRTRestarter) Restart() error {
	...
	if err := exec.Command("/etc/init.d/BIGBROTHER-agent", "running").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting BIGBROTHER-agent via procd…")
	return exec.Command("/etc/init.d/BIGBROTHER-agent", "restart").Run()
}

type freeBSDRestarter struct{ cmd string }

func (f *freeBSDRestarter) Restart() error {
	if err := exec.Command(f.cmd, "BIGBROTHER-agent", "status").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting BIGBROTHER-agent via FreeBSD rc…")
	return exec.Command(f.cmd, "BIGBROTHER-agent", "restart").Run()
}
...
// Update checks GitHub for a newer release of BIGBROTHER-agent, applies it,
...
func Update(useMirror bool) error {
	...
	updated, err := ghupdate.Update(ghupdate.Config{
		ArchiveExecutable: "BIGBROTHER-agent",
		DataDir:           dataDir,
		UseMirror:         useMirror,
	})
	if err != nil {
	...
	// set ownership to BIGBROTHER:BIGBROTHER if possible
	if chownPath, err := exec.LookPath("chown"); err == nil {
		if err := exec.Command(chownPath, "BIGBROTHER:BIGBROTHER", exePath).Run(); err != nil {
