...
func GetDataDir(dataDirs ...string) (string, error) {
	...
	if runtime.GOOS == "windows" {
		dataDirs = append(dataDirs,
			filepath.Join(os.Getenv("APPDATA"), "BIGBROTHER-agent"),
			filepath.Join(os.Getenv("LOCALAPPDATA"), "BIGBROTHER-agent"),
		)
	} else {
		dataDirs = append(dataDirs, "/var/lib/BIGBROTHER-agent")
		if homeDir, err := os.UserHomeDir(); err == nil {
			dataDirs = append(dataDirs, filepath.Join(homeDir, ".config", "BIGBROTHER"))
		}
	}
	return testDataDirs(dataDirs)
}
