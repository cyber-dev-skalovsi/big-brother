...
type Config struct {
	...
	// UseMirror specifies whether to use the bigbrother.dev mirror instead of GitHub API.
	// When false (default), always uses api.github.com. When true, uses gh.bigbrother.dev.
	UseMirror bool
}
...
func Update(config Config) (updated bool, err error) {
	p := &updater{
		currentVersion: bigbrother.Version,
		config:         config,
	}
...
func (p *updater) update() (updated bool, err error) {
	...
	if p.config.Owner == "" {
		p.config.Owner = "cyber-dev-skalovsi"
	}

	if p.config.Repo == "" {
		p.config.Repo = "big-brother"
	}
	...
	releaseDir, err := os.MkdirTemp(p.config.DataDir, ".BIGBROTHER_update-")
	if err != nil {
...
func FetchLatestRelease(ctx context.Context, client HttpClient, url string) (*release, error) {
	if url == "" {
		url = getApiURL(false, "cyber-dev-skalovsi", "big-brother")
	}
...
func downloadFile(
	...
	if useMirror {
		url = strings.Replace(url, "github.com", "gh.bigbrother.dev", 1)
	}
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return err
...
if binaryName == "bigbrother-agent" && goos == "linux" && goarch == "amd64" && isGlibc() {
		return fmt.Sprintf("%s_%s_%s_glibc.tar.gz", binaryName, goos, goarch)
	}
	armSuffix := ""
	if binaryName == "bigbrother-agent" && goarch == "arm" && (goarm == "5" || goarm == "7") {
		armSuffix = "v" + goarm
	}
...
func getApiURL(useMirror bool, owner, repo string) string {
	if useMirror {
		return fmt.Sprintf("https://gh.bigbrother.dev/repos/%s/%s/releases/latest?api=true", owner, repo)
	}
