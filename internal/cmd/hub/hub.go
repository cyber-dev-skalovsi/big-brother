...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/internal/hub"
	_ "github.com/cyber-dev-skalovsi/big-brother/internal/migrations"
...
func getBaseApp() *pocketbase.PocketBase {
	...
	baseApp := pocketbase.NewWithConfig(pocketbase.Config{
		DefaultDataDir: bigbrother.AppName + "_data",
		DefaultDev:     isDev,
	})
	baseApp.RootCmd.Version = bigbrother.Version
	baseApp.RootCmd.Use = bigbrother.AppName
	baseApp.RootCmd.Short = ""
	// add update command
	updateCmd := &cobra.Command{
		Use:   "update",
		Short: "Update " + bigbrother.AppName + " to the latest version",
		Run:   hub.Update,
	}
	updateCmd.Flags().Bool("china-mirrors", false, "Use mirror (gh.bigbrother.dev) instead of GitHub")
