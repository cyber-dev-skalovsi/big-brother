...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/agent"
	"github.com/cyber-dev-skalovsi/big-brother/agent/health"
	"github.com/cyber-dev-skalovsi/big-brother/agent/utils"
...
type cmdOptions struct {
	...
	hubURL string // hubURL is the URL of the Big Brother hub.
...
func (opts *cmdOptions) parse() bool {
	...
	pflag.StringVarP(&opts.hubURL, "url", "u", "", "URL of the Big Brother hub")
	pflag.StringVarP(&opts.token, "token", "t", "", "Token to use for authentication")
	chinaMirrors := pflag.BoolP("china-mirrors", "c", false, "Use mirror for update (gh.bigbrother.dev) instead of GitHub")
	...
	switch {
	case *version:
		fmt.Println(bigbrother.AppName+"-agent", bigbrother.Version)
		return true
...
func (opts *cmdOptions) loadPublicKeys() ([]ssh.PublicKey, error) {
	...
	if !ok {
		return nil, fmt.Errorf("no key provided: must set -key flag, KEY env var, or KEY_FILE env var. Use 'bigbrother-agent help' for usage")
	}
