...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/internal/hub/transport"
	"github.com/cyber-dev-skalovsi/big-brother/internal/hub/utils"
	"github.com/cyber-dev-skalovsi/big-brother/internal/hub/ws"
...
func (sys *System) FetchSmartDataFromAgent() (smart.SmartDataResponse, error) {
	...
	defer cancel()
	if sys.agentVersion.LT(bigbrother.MinVersionAgentResponse) {
...
func (sys *System) fetchDataViaSSH(options common.DataRequestOptions) (*system.CombinedData, error) {
	err := sys.runSSHOperation(4*time.Second, 1, func(session *ssh.Session) (bool, error) {
		...
		*sys.data = system.CombinedData{}
		...
		if sys.agentVersion.GTE(bigbrother.MinVersionAgentResponse) && stdinErr == nil {
		...
		var decodeErr error
		if sys.agentVersion.GTE(bigbrother.MinVersionCbor) {
...
// extractAgentVersion extracts the BIGBROTHER version from SSH server version string
