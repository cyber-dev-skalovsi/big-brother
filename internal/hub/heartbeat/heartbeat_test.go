...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/internal/hub/heartbeat"
...
func TestSendGETDoesNotRequireAppOrDB(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "BIGBROTHER-Heartbeat", r.Header.Get("User-Agent"))
...
func TestSendPOSTBuildsExpectedStatuses(t *testing.T) {
	tests := []struct {
		name           string
		setup          func(t *testing.T, app *BIGBROTHERtests.TestHub, user *core.Record)
		expectStatus   string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "BIGBROTHER-Heartbeat", req.userAgent)
...
func newTestHub(t *testing.T) *BIGBROTHERtests.TestHub {
	t.Helper()
	app, err := BIGBROTHERtests.NewTestHub(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(app.Cleanup)
	return app
}

func createTestUser(t *testing.T, app *BIGBROTHERtests.TestHub) *core.Record {
	t.Helper()
	user, err := BIGBROTHERtests.CreateUser(app.App, "admin@example.com", "password123")
	require.NoError(t, err)
	return user
}

func createTestSystem(t *testing.T, app *BIGBROTHERtests.TestHub, userID, name, host, status string) *core.Record {
	t.Helper()
	system, err := BIGBROTHERtests.CreateRecord(app.App, "systems", map[string]any{
		"name":   name,
		"host":   host,
		"port":   "45876",
		"users":  []string{userID},
...
func createTriggeredAlert(t *testing.T, app *BIGBROTHERtests.TestHub, userID, systemID, name string, threshold float64) *core.Record {
	t.Helper()
	alert, err := BIGBROTHERtests.CreateRecord(app.App, "alerts", map[string]any{
		"name":      name,
		"system":    systemID,
		"user":      userID,
		"value":     threshold,
