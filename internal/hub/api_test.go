...
import (
	...
	BIGBROTHERTests "github.com/cyber-dev-skalovsi/big-brother/internal/tests"

	"github.com/cyber-dev-skalovsi/big-brother/internal/migrations"
...
func TestApiRoutesAuthentication(t *testing.T) {
	hub, user := BIGBROTHERTests.GetHubWithUser(t)
	defer hub.Cleanup()
	...
	user2, err := BIGBROTHERTests.CreateUser(hub, "testuser@example.com", "password123")
	require.NoError(t, err, "Failed to create test user")
	user2Token, err := user2.NewAuthToken()
	require.NoError(t, err, "Failed to create user2 auth token")

	adminUser, err := BIGBROTHERTests.CreateUserWithRole(hub, "admin@example.com", "password123", "admin")
	require.NoError(t, err, "Failed to create admin user")
	adminUserToken, err := adminUser.NewAuthToken()

	readOnlyUser, err := BIGBROTHERTests.CreateUserWithRole(hub, "readonly@example.com", "password123", "readonly")
	...
	superuser, err := BIGBROTHERTests.CreateSuperuser(hub, "superuser@example.com", "password123")
	...
	// Create test system
	system, err := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "test-system",
	...
	scenarios := []BIGBROTHERTests.ApiScenario{
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/bigbrother/heartbeat-status",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		...
		},
		{
			...
			URL:    "/api/bigbrother/test-heartbeat",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  403,
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/bigbrother/universal-token",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/universal-token?enable=1&permanent=1&token=permanent-token-123",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  200,
		...
		},
		{
			...
			Method: http.MethodPost,
			URL:    "/api/bigbrother/smart/refresh",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodPost,
			URL:    fmt.Sprintf("/api/bigbrother/smart/refresh?system=%s", system.Id),
			Headers: map[string]string{
				"Authorization": readOnlyUserToken,
			},
			ExpectedStatus:  403,
		...
		},
		{
			...
			Method:          http.MethodPost,
			URL:             "/api/bigbrother/user-alerts",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		...
		},
		{
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				...
				BIGBROTHERTests.CreateRecord(app, "alerts", map[string]any{
					"name":   "CPU",
					"system": system.Id,
					"user":   user.Id,
					"value":  80,
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/bigbrother/containers/logs?system=test-system&container=abababababab",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		{
			...
			Method:          http.MethodGet,
			URL:             fmt.Sprintf("/api/bigbrother/containers/logs?system=%s&container=abababababab", system.Id),
			ExpectedStatus:  404,
			ExpectedContent: []string{"The requested resource wasn't found."},
			TestAppFactory:  testAppFactory,
			Headers: map[string]string{
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             fmt.Sprintf("/api/bigbrother/containers/info?system=%s&container=abababababab", system.Id),
			ExpectedStatus:  404,
			ExpectedContent: []string{"The requested resource wasn't found."},
			TestAppFactory:  testAppFactory,
			Headers: map[string]string{
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/logs?container=abababababab",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/logs?system=test-system",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/logs?system=invalid-system&container=0123456789ab",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  404,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/logs?system=" + system.Id + "&container=..%2F..%2Fversion",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/info?system=" + system.Id + "&container=../../version?x=",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/info?system=" + system.Id + "&container=container_name",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/logs?system=" + system.Id + "&container=0123456789ab",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  500,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/containers/info?system=" + system.Id + "&container=0123456789ab",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  500,
		...
		// /systemd routes
		{
			...
			Method:          http.MethodGet,
			URL:             fmt.Sprintf("/api/bigbrother/systemd/info?system=%s&service=nginx.service", system.Id),
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/systemd/info?service=nginx.service",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    fmt.Sprintf("/api/bigbrother/systemd/info?system=%s", system.Id),
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/bigbrother/systemd/info?system=invalid-system&service=nginx.service",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  404,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    fmt.Sprintf("/api/bigbrother/systemd/info?system=%s&service=notregistered.service", system.Id),
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  404,
		...
		},
		{
			...
			TestAppFactory:  testAppFactory,
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				BIGBROTHERTests.CreateRecord(app, "systemd_services", map[string]any{
					"system": system.Id,
					"name":   "nginx.service",
					"state":  0,
					"sub":    1,
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/bigbrother/agent-connect",
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodPost,
			URL:    "/api/bigbrother/test-notification",
			Body: jsonReader(map[string]any{
...
func TestFirstUserCreation(t *testing.T) {
	t.Run("CreateUserEndpoint available when no users exist", func(t *testing.T) {
		hub, _ := BIGBROTHERTests.NewTestHub(t.TempDir())
		defer hub.Cleanup()
		...
		scenarios := []BIGBROTHERTests.ApiScenario{
			{
				...
				Method: http.MethodPost,
				URL:    "/api/bigbrother/create-user",
				Body: jsonReader(map[string]any{
	...
	t.Run("CreateUserEndpoint not available when USER_EMAIL, USER_PASSWORD are set", func(t *testing.T) {
		t.Setenv("BIGBROTHER_HUB_USER_EMAIL", "me@example.com")
		t.Setenv("BIGBROTHER_HUB_USER_PASSWORD", "password123")
		...
		scenario := BIGBROTHERTests.ApiScenario{
...
func TestCreateUserEndpointAvailability(t *testing.T) {
	...
	t.Run("CreateUserEndpoint not available when users exist", func(t *testing.T) {
		...
		// Create a user first
		_, err := BIGBROTHERTests.CreateUser(hub, "existing@example.com", "password")
		require.NoError(t, err)
...
func TestAutoLoginMiddleware(t *testing.T) {
	var hubs []*BIGBROTHERTests.TestHub
	...
	scenarios := []BIGBROTHERTests.ApiScenario{
		...
		},
		{
			...
			TestAppFactory:  testAppFactory,
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				BIGBROTHERTests.CreateUser(app, "user@test.com", "password123")
			},
		},
	}
...
func TestTrustedHeaderMiddleware(t *testing.T) {
	...
	t.Setenv("TRUSTED_AUTH_HEADER", "X-BIGBROTHER-Trusted")
	...
	scenarios := []BIGBROTHERTests.ApiScenario{
		...
		},
		{
			...
			URL:    "/api/bigbrother/getkey",
			Headers: map[string]string{
				"X-BIGBROTHER-Trusted": "user@test.com",
			},
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
...
func TestUpdateEndpoint(t *testing.T) {
	...
	// Create test user and get auth token
	// user, err := BIGBROTHERTests.CreateUser(hub, "testuser@example.com", "password123")
	// require.NoError(t, err, "Failed to create test user")
	// userToken, err := user.NewAuthToken()
