...
export default function HeartbeatSettings() {
	...
	async function fetchStatus() {
		try {
			setIsLoading(true)
			const res = await pb.send<HeartbeatStatus>("/api/bigbrother/heartbeat-status", {})
			setStatus(res)
		} catch (error: unknown) {
			toast({
				title: t`Error`,
	...
	async function sendTestHeartbeat() {
		setIsTesting(true)
		try {
			const res = await pb.send<{ err: string | false }>("/api/bigbrother/test-heartbeat", {
				method: "POST",
			})
		...
		<div>
			<div>
				...
				<p className="text-sm text-muted-foreground leading-relaxed">
					<Trans>
						Send periodic outbound pings to an external monitoring service so you can monitor BIGBROTHER without exposing it
						to the internet.
					</Trans>
				</p>
			</div>
...
function NotEnabledState({ isLoading }: { isLoading?: boolean }) {
	return (
		<div className={cn("grid gap-4", isLoading && "animate-pulse")}>
			<div>
				<p className="text-sm text-muted-foreground leading-relaxed mb-3">
					<Trans>Set the following environment variables on your BIGBROTHER hub to enable heartbeat monitoring:</Trans>
				</p>
			...
			<p className="text-sm text-muted-foreground leading-relaxed">
				<Trans>After setting the environment variables, restart your BIGBROTHER hub for changes to take effect.</Trans>
			</p>
		</div>
	)
}
