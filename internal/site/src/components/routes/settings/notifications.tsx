					...
					<div className="grid grid-cols-1 sm:flex items-center justify-between gap-4">
						<div>
							...
							<p className="text-sm text-muted-foreground leading-relaxed">
								<Trans>
									BIGBROTHER uses{" "}
									<a href="https://github.com/cyber-dev-skalovsi/big-brother/guide/notifications" target="_blank" className="link" rel="noopener">
										Shoutrrr
									</a>{" "}
...
const ShoutrrrUrlCard = ({ url, onUrlChange, onRemove }: ShoutrrrUrlCardProps) => {
	...
	const sendTestNotification = async () => {
		setIsLoading(true)
		try {
			const res = await pb.send("/api/bigbrother/test-notification", { method: "POST", body: { url } })
