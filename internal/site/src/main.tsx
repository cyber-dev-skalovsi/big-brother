...
import type { BIGBROTHERInfo, UpdateInfo } from "./types"
...
const App = memo(() => {
	...
	useEffect(() => {
		...
		pb.send<BIGBROTHERInfo>("/api/bigbrother/info", {}).then((data) => {
			$publicKey.set(data.key)
			// check for updates if enabled
			if (data.cu && isAdmin()) {
				pb.send<UpdateInfo>("/api/bigbrother/update", {}).then($newVersion.set)
			}
		})
		// get user settings
		updateUserSettings()
