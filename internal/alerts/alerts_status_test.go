...
import (
	...
	"github.com/cyber-dev-skalovsi/big-brother/internal/alerts"
...
func TestStatusAlerts(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		systems, err := BIGBROTHERTests.CreateSystems(hub, 4, user.Id, "paused")
		assert.NoError(t, err)

		var alerts []*core.Record
		for i, system := range systems {
			alert, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
				"name":   "Status",
				"system": system.Id,
				"user":   user.Id,
				"min":    i + 1,
...
func TestStatusAlertMultipleUsersRespectDifferentMinutes(t *testing.T) {
	hub, user1 := BIGBROTHERTests.GetHubWithUser(t)
	...
	synctest.Test(t, func(t *testing.T) {
		...
		user2, err := BIGBROTHERTests.CreateUser(hub, "user2@example.com", "password")
		require.NoError(t, err)
		_, err = BIGBROTHERTests.CreateRecord(hub, "user_settings", map[string]any{
			"user": user2.Id,
		...
		alertUser1, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system.Id,
			"user":   user1.Id,
			"min":    1,
		...
		require.NoError(t, err)
		alertUser2, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system.Id,
			"user":   user2.Id,
			"min":    2,
...
func TestRestorePendingStatusAlertsRequeuesDownSystemsAfterRestart(t *testing.T) {
	...
	systems, err := BIGBROTHERTests.CreateSystems(hub, 1, user.Id, "down")
	require.NoError(t, err)
	system := systems[0]
...
func TestRestorePendingStatusAlertsSkipsNonDownOrAlreadyTriggeredAlerts(t *testing.T) {
	...
	systemsDown, err := BIGBROTHERTests.CreateSystems(hub, 2, user.Id, "down")
	require.NoError(t, err)
	systemDownPending := systemsDown[0]
	systemDownTriggered := systemsDown[1]

	systemUp, err := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":   "up-system",
	...
	_, err = BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
		"name":      "Status",
...
func TestResolveStatusAlertsFixesStaleTriggered(t *testing.T) {
	...
	systems, err := BIGBROTHERTests.CreateSystems(hub, 1, user.Id, "up")
	require.NoError(t, err)
	system := systems[0]
...
func TestResolveStatusAlerts(t *testing.T) {
	...
	systemDown, err := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":   "test-system-2",
		"users":  []string{user.Id},
		"host":   "127.0.0.2",
		"status": "up",
	...
	alertUp, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
		"name":   "Status",
		"system": systemUp.Id,
		"user":   user.Id,
		"min":    1,
	...
	alertDown, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
		"name":   "Status",
...
func TestAlertsHistoryStatus(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		alertRecord, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system.Id,
			"user":   user.Id,
			"min":    1,
