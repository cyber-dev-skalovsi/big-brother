...
func TestAlertSilencedOneTime(t *testing.T) {
	...
	_, err = BIGBROTHERTests.CreateRecord(hub, "quiet_hours", map[string]any{
		"user":   user.Id,
		"system": system.Id,
		"type":   "one-time",
		"start":  startTime,
...
func TestAlertSilencedGlobal(t *testing.T) {
	...
	systems, err := BIGBROTHERTests.CreateSystems(hub, 3, user.Id, "up")
	assert.NoError(t, err)
...
func TestAlertSilencedSystemSpecific(t *testing.T) {
	...
	// Create multiple systems
	systems, err := BIGBROTHERTests.CreateSystems(hub, 2, user.Id, "up")
	assert.NoError(t, err)
	system1 := systems[0]
	system2 := systems[1]
...
func TestAlertSilencedMultiUser(t *testing.T) {
	hub, _ := BIGBROTHERTests.GetHubWithUser(t)
	defer hub.Cleanup()

	// Create two users
	user1, err := BIGBROTHERTests.CreateUser(hub, "user1@example.com", "password")
	assert.NoError(t, err)
...
func TestAlertSilencedWithActualAlert(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		if err != nil || userSettings == nil {
			userSettings, err = BIGBROTHERTests.CreateRecord(hub, "user_settings", map[string]any{
				"user": user.Id,
