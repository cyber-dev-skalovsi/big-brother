...
func TestBatteryAlertLogic(t *testing.T) {
	...
	batteryAlert, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
		"name":   "Battery",
	...
	_, err = BIGBROTHERTests.CreateRecord(hub, "system_stats", map[string]any{
		"system": systemRecord.Id,
		"type":   "1m",
...
func TestBatteryAlertAveragedSamples(t *testing.T) {
	...
	for _, offset := range recordTimes {
		...
		record, err := BIGBROTHERTests.CreateRecord(hub, "system_stats", map[string]any{
			"system": systemRecord.Id,
			"type":   "1m",
			"stats":  string(statsLowJSON),
		})
