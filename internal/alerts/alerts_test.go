...
func TestAlertsHistory(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		systems2, err := BIGBROTHERTests.CreateSystems(hub, 1, user.Id, "up")
		assert.NoError(t, err)
		system2 := systems2[0]
		...
		alert2, err := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system2.Id,
			"user":   user.Id,
			"min":    1,
...
func TestSetAlertTriggered(t *testing.T) {
	...
	user, _ := BIGBROTHERTests.CreateUser(hub, "test@example.com", "password")
	system, _ := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "test-system",
		"users": []string{user.Id},
		"host":  "127.0.0.1",
	})

	alertRecord, _ := BIGBROTHERTests.CreateRecord(hub, "alerts", map[string]any{
		"name":      "CPU",
		"system":    system.Id,
		"user":      user.Id,
		"value":     80,
