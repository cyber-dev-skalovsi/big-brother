...
func TestIsInternalURL(t *testing.T) {
	testCases := []struct {
		...
		{name: "localhost hostname", url: "generic+https://bigbrother.dev/api/v1/postStuff", internal: false},
...
func TestUserAlertsApi(t *testing.T) {
	...
	user1, _ := BIGBROTHERTests.CreateUser(hub, "alertstest@example.com", "password")
	user1Token, _ := user1.NewAuthToken()

	user2, _ := BIGBROTHERTests.CreateUser(hub, "alertstest2@example.com", "password")
	user2Token, _ := user2.NewAuthToken()

	system1, _ := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "system1",
		"users": []string{user1.Id},
		"host":  "127.0.0.1",
	})

	system2, _ := BIGBROTHERTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "system2",
	...
	scenarios := []BIGBROTHERTests.ApiScenario{
		...
		// 	ExpectedStatus:  200,
		// 	ExpectedContent: []string{"<html ", "globalThis.BIGBROTHER"},
		...
		},
		{
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				BIGBROTHERTests.ClearCollection(t, app, "alerts")
		...
		},
		{
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				...
				for _, systemId := range []string{system1.Id, system2.Id} {
					_, err := BIGBROTHERTests.CreateRecord(app, "alerts", map[string]any{
						"name":   "Memory",
						"system": systemId,
						"user":   user1.Id,
						"value":  90,
